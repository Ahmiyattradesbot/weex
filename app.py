...
def find_free_port() -> int:
    """Find a free port for a new bot-engine instance. Actually checks port availability."""
    used_ports = set()
    ...
    for port in range(PORT_START, PORT_END + 1):
        if port in used_ports:
            continue
        try:
            with _sock.create_server(("127.0.0.1", port), reuse_port=False):
                return port
        except OSError:
            continue
    ...
    try:
        ...
        for i in range(10):
            ...
            if proc.poll() is not None:
                logger.error(f"Bot-engine process crashed with return code {proc.returncode}")
                DB.get("bot_processes", {}).pop(user_id, None)
                save_db(DB)
                return {"success": False, "error": f"Bot-engine crashed (exit code {proc.returncode}). Check logs."}
...
def api_bot_ensure_running():
    ...
    # Always write config before starting — ensures config.json exists even on fresh deploy (Railway)
    ...
    if not result.get("success"):
        logger.error(f"ensure_running failed for {user['id']}: {result.get('error')}")
    return jsonify(result)
...
ROUTES_NEED_BOT = {"start", "stop", "force_stop", "close", "price",
                   "test_chart", "preview", "test_connection",
                   "test_notification", "active_symbol", "balance"}


def _save_config_to_db(user, body):
    """Save config POST body to SaaS DB (encrypted)."""
...
def _handle_config_get(user):
    """Serve /api/config directly from SaaS DB — no bot-engine needed."""
    config = user.get("bot_config", {})
    api_key = decrypt(config.get("api_key_enc", ""))
    api_secret = decrypt(config.get("api_secret_enc", ""))
    return jsonify({
        ...
        "take_profit_pct": config.get("take_profit_pct", 6),
        "mode": config.get("mode", "both"),
        "running": get_bot_status(user["id"]).get("running", False),
    })


def _handle_status(user):
    """Serve /api/status directly — no bot-engine needed."""
    bot_status = get_bot_status(user["id"])
    config = user.get("bot_config", {})
    return jsonify({
...
def _handle_symbols(user):
    """Fetch symbols from exchange directly — no bot-engine needed."""
    config = user.get("bot_config", {})
    ...
    if exchange == "weex":
        try:
            resp = req_lib.get("https://api-contract.weex.com/capi/v3/market/apiTradingSymbols", timeout=10)
            data = resp.json()
            all_symbols = []
            ...
            if testnet:
                ...
                symbols = [s for s in all_symbols if s.endswith("USDT") and not s.endswith("SUSDT")]
            if symbols:
                return jsonify({"success": True, "symbols": symbols, "count": len(symbols),
                                "exchange": "weex", "mode": "demo" if testnet else "live"})
        except Exception as e:
            logger.warning(f"WEEX symbols fetch failed: {e}")
        fallback = ["BTCSUSDT", "ETHSUSDT", "BNBSUSDT", "SOLSUSDT", "XRPSUSDT"] if testnet else \
                   ["BTCUSDT", "ETHUSDT", "BNBUSDT", "SOLUSDT", "XRPUSDT"]
        return jsonify({"success": True, "symbols": fallback, "count": len(fallback),
                        "exchange": "weex", "mode": "demo" if testnet else "live"})

    try:
        resp = req_lib.get("https://fapi.binance.com/fapi/v1/exchangeInfo", timeout=10)
        data = resp.json()
        symbols = []
        ...
        symbols.sort()
        if symbols:
            return jsonify({"success": True, "symbols": symbols, "count": len(symbols), "exchange": "binance"})
    except Exception as e:
        logger.warning(f"Binance symbols fetch failed: {e}")
...
def bot_api_proxy(path):
    ...
    if first_segment == "config":
        if request.method == "GET":
            return _handle_config_get(user)
        else:
            body = request.get_json(force=True, silent=True) or {}
            _save_config_to_db(user, body)
            return jsonify({"success": True})

    if first_segment == "status":
        return _handle_status(user)

    if first_segment == "symbols":
    ...
    if not status["running"] or not status.get("port"):
        if first_segment in ("start", "balance"):
            logger.info(f"Auto-starting bot-engine for {user['id'][:8]} (route: {first_segment})")
            write_user_bot_config(user["id"])
            start_result = start_user_bot(user["id"], write_config=False)
            if not start_result.get("success"):
                return jsonify({"success": False, "error": f"Bot start nahi ho paya: {start_result.get('error', 'Unknown')}"}), 500
            status = get_bot_status(user["id"])
            if not status.get("running"):
                return jsonify({"success": False, "error": "Bot start ho raha hai, thodi der baad try karein."}), 500
        else:
            return jsonify({"success": False, "error": "Bot engine not running."}), 400

    if first_segment == "balance":
        target_url = f"http://127.0.0.1:{status['port']}/api/balance"
        try:
